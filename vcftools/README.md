...
- --BEAGLE-GL   
- --BEAGLE-PL   
这些选项输出基因型可能性信息以输入到 BEAGLE 程序中。 VCF 文件需要包含带有“GL”或“PL”标记的 FORMAT 字段，这些字段通常可以由 SNP 调用程序（例如 GATK）输出。使用此选项需要通过“--chr”选项指定染色体。生成的输出文件具有后缀“.BEAGLE.GL”或“.BEAGLE.PL”，并包含双等位基因位点的基因型可能性。该文件适合通过“like=”参数输入 BEAGLE。

- --plink   
- --plink-tped  
- --chrom-map   
这些选项以 PLINK PED 格式输出基因型数据。使用第一个选项，会生成两个文件，后缀为“.ped”和“.map”。请注意，仅输出双等位基因座。这些文件的更多详细信息可以在 PLINK 文档中找到。
...
- --diff-indv   
输出每个文件共有/唯一的个体。输出文件的后缀为“.diff.indv_in_files”。

- --diff-site-discordance   
此选项计算逐个站点的不一致。生成的输出文件具有后缀“.diff.sites”。

- --diff-indv-discordance   
此选项计算每个个体的不一致性。生成的输出文件具有后缀“.diff.indv”。

- --diff-indv-map `filename`   
该选项允许用户指定第二个文件中的各个 ID 到第一个文件中的各个 ID 的映射。该程序期望该文件包含一个制表符分隔的行，该行包含文件一中的个人姓名，后跟文件二中的同一个人的姓名，每行一个映射。 

- --diff-discordance-matrix  
此选项计算不一致矩阵。此选项仅适用于两个文件中都存在匹配等位基因的双等位基因位点。生成的输出文件具有后缀“.diff.discordance.matrix”。

- --diff-switch-error    
此选项计算相位误差（特别是“开关误差”）。此选项创建一个输出文件，描述站点之间发现的切换错误，后缀为“.diff.switch”。  
